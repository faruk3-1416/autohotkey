...
cornerY := A_ScreenHeight - cornerBuffer

; Define the top-left corner
cornerX_Left := cornerBuffer
cornerY_Top := cornerBuffer

; Define variables to track whether the Task View has been activated and whether the mouse is in the top-left corner
...
Loop {
    ...
    if (mouseX <= cornerX_Left && mouseY <= cornerY_Top) {
        SwitchToPreviousWindow()
        taskViewActivated := false
